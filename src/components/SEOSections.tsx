  ...
  return (
    <section className="w-full max-w-5xl mx-auto py-16 md:py-24 px-4">
      <div className="space-y-6 text-center mb-12 md:mb-16">
        <h2 className="text-3xl md:text-5xl font-black tracking-tighter text-foreground uppercase">Base64 Converter — Professional Developer Workflow</h2>
        <p className="text-base md:text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed font-medium px-2">
          Forge Studios is a free, fast Base64 converter and image encoder designed for modern asset delivery pipelines. 
          Convert PNG to Base64, JPEG to Base64, WebP to Base64, and SVG to Base64 Data URI strings — or use the lossless Base64 decoder to restore images instantly, with zero HTTP overhead.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 mt-12">
        <div className="glass-card p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border border-foreground/5 flex flex-col h-full">
          <h3 className="text-xl md:text-2xl font-black mb-6 tracking-tight">How to use the Base64 Encoder</h3>
          <div className="space-y-6 text-sm text-muted-foreground leading-relaxed flex-1">
            <div className="space-y-2">
              <p className="font-bold text-foreground">1. HTML Image Encoder</p>
            ...
            <div className="space-y-2">
              <p className="font-bold text-foreground">2. CSS Image Encoder</p>
              <code className="block p-4 bg-foreground/5 rounded-xl font-code text-[10px] md:text-xs overflow-x-auto whitespace-pre-wrap break-all">
        ...
        <div className="glass-card p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border border-foreground/5 flex flex-col h-full overflow-hidden">
          <h3 className="text-xl md:text-2xl font-black mb-6 tracking-tight">Base64 vs URL — Performance Comparison</h3>
...
export function FAQSection() {
  const faqs = [
    {
      q: "What is the 33% overhead rule for Base64 converters?",
      a: "Base64 encoding increases file size by approximately 33% because it uses 4 characters to represent every 3 bytes of binary data. We recommend using Forge's online Base64 tool only for assets under 10KB (like icons or logos) where the elimination of an HTTP request outweighs the size increase."
    },
    {
      q: "When should I convert PNG to WebP Base64?",
      a: "Always, if your target environment supports modern browsers. Our fast Base64 converter outputs WebP Base64 strings that are significantly shorter than PNG Base64 strings, often offsetting the 33% encoding overhead entirely — making it the best lossless Base64 strategy."
    },
    {
      q: "Is batch Base64 conversion limited?",
      a: "No, but browser memory is. We recommend processing batches of 10-20 assets at a time to maintain UI smoothness (120Hz performance). Our Base64 string generator handles PNG, JPEG, WebP, SVG, and GIF in a single batch."
    },
    {
      q: "How does using a Base64 converter impact SEO?",
      a: "By reducing HTTP requests, your page's First Contentful Paint (FCP) can improve, which is a key Core Web Vital for Google ranking. However, overusing large Base64 strings can bloat your HTML and delay overall parsing. Use our web performance tool to find the optimal balance."
    }
  ]

  return (
    <section className="w-full max-w-4xl mx-auto py-16 md:py-24 px-4 border-t border-foreground/5">
      <h2 className="text-2xl md:text-3xl font-black text-center mb-10 md:mb-12 tracking-tighter uppercase">Base64 Converter — FAQ</h2>
